...
struct BatchLookupNode : public ParameterNodeBase {
  BatchLookupNode(LookupParameters* p, const std::vector<unsigned>& indices) : dim(p->dim, indices.size()), indices(indices), pindices(&this->indices), params(p) {}
  BatchLookupNode(LookupParameters* p, const std::vector<unsigned>* pindices) : dim(p->dim, pindices->size()), indices(), pindices(pindices), params(p) {}
  std::string as_string(const std::vector<std::string>& arg_names) const override;
  Dim dim_forward(const std::vector<Dim>& xs) const override;
  void forward_impl(const std::vector<const Tensor*>& xs, Tensor& fx) const override;
  void backward_impl(const std::vector<const Tensor*>& xs,
